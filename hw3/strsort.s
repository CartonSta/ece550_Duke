...
#   s4 is array[0], s0 is array[j]
#   s1 is length, s2 = j, s3 = i
...
.Out_loop:
    move $s0, $s4   #  s4 is array[0], s0 is array[j]
    addiu $s3, $s3, 1
    move $s2, $s3
    blt $s3, $s1, .Sort_lp  # s1 is length, s2 = i, s3 = j
